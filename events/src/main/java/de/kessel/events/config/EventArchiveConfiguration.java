...
public class EventArchiveConfiguration {
    ...
    CommandLineRunner initDatabase(EventMetadataRepository eventMetadataRepository) {
        return args -> {
            ...
            Flux<EventMetadataResponseDto> allEventMetadatasForYearAsc = eventMetadataService.findAllByOrderByYearAsc();
            Flux<EventMetadataResponseDto> allEventMetadatasForYearDesc = eventMetadataService.findAllByOrderByYearDesc();
            ...
            eventMetadataService.deleteAllEventMetadatas()
                    ...
                    .thenMany(allEventMetadatas.count().doOnNext(count -> assertThat(count).isEqualTo(16)))
                    .thenMany(allEventMetadatasForAyear.count().doOnNext(count -> assertThat(count).isEqualTo(6)))
                    .thenMany(allEventMetadatasForYearAndMonth.count().doOnNext(count -> assertThat(count).isEqualTo(2)))
                    .thenMany(allEventMetadatasForYearAsc.takeLast(1).doOnNext(metadata -> assertThat(metadata.getDate()).isEqualTo("1. Februar 2028 n. Chr.")))
                    .thenMany(allEventMetadatasForYearDesc.takeLast(1).doOnNext(metadata -> assertThat(metadata.getDate()).isEqualTo(" Januar 200 v. Chr.")))
                    .subscribe();
    ...
    private List<EventMetadataRequestDto> createEventMetadataRequestDtos() {
        ...
        var emr6 = EventMetadataRequestDto.builder().year(-200).month(-1).day(-1).eventId("006").build();
